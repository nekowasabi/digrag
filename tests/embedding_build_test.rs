...
fn test_index_builder_with_embeddings_creates_client() {
    ...
    assert!(
        builder.has_embedding_client(),
        "Builder should have embedding client"
    );
}
...
fn test_index_builder_without_embeddings_no_client() {
    ...
    assert!(
        !builder.has_embedding_client(),
        "Builder should not have embedding client"
    );
}
...
async fn test_build_with_embeddings_creates_vector_index() {
    ...
    std::fs::write(
        &input_path,
        r#"* Test Entry 2025-01-15 10:00:00 [memo]:
...
"#,
    ...
    let builder =
        IndexBuilder::with_embeddings_and_base_url("test-api-key".to_string(), mock_server.uri());

    // Build with embeddings
    let result = builder
        .build_with_embeddings(&input_path, &output_dir, |step, total, msg| {
    ...
    assert!(
        result.is_ok(),
        "build_with_embeddings should succeed: {:?}",
        result.err()
    );
    ...
    assert!(
        ...
        "Vector index should have dimension"
    );
}
...
async fn test_build_with_embeddings_reports_progress() {
    ...
    std::fs::write(
        &input_path,
        r#"* Entry 1 2025-01-15 10:00:00 [memo]:
  Content 1
...
"#,
    ...
    let result = builder
        .build_with_embeddings(&input_path, &output_dir, move |step, total, msg| {
            progress_steps_clone
                .lock()
                .unwrap()
                .push((step, total, msg.to_string()));
        })
        .await;
    ...
    let has_embedding_step = steps
        .iter()
        .any(|(_, _, msg)| msg.contains("embedding") || msg.contains("Embedding"));
    assert!(
        has_embedding_step,
        "Should report embedding generation progress"
    );
}
...
async fn test_openrouter_api_mock_success() {
    ...
    let client = OpenRouterEmbedding::with_base_url("test-api-key".to_string(), mock_server.uri());
...
"#,
    ...
    assert!(
        json.get("dimension").is_some(),
        "Should have dimension field"
    );
